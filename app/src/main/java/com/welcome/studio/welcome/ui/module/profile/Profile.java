...
import com.welcome.studio.welcome.ui.adapter.ProfileAdapter;
...
public class Profile extends BaseMainFragment implements ProfileView {
    ...
    public static Profile newInstance(User user, boolean isAddedToBackStack) {
        Profile profile = new Profile();
        Bundle args = new Bundle();
        args.putSerializable("user", user);
        args.putBoolean("is_added_to_back_stack",isAddedToBackStack);
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        adapter = new ProfileAdapter(getChildFragmentManager(),
                getResources().getString(R.string.now), getResources().getString(R.string.history), user);
    ...
    public boolean isAddedToBackStack() {
        return getArguments().getBoolean("is_added_to_back_stack", false);
    }
    ...
    @Override
    public void loadMainPhoto(Uri uri) {
        Picasso.with(getContext()).load(R.drawable.toolbar_background).into(imgToolbarBackground);
        Picasso.with(getContext()).load(uri).error(R.mipmap.img_avatar).into(mainPhoto);
    }
